...
import sys
sys.path.append('/home/siyuan/pytorch-semantic-segmentation-master')
...
sorce_path = '/home/pan_member/fat'
ckpt_path = '/home/pan_member/ckpt'
exp_name = 'ot-unet'
...
def main(train_args):
    print('start!')
    net = UNet(num_classes=ot.num_classes).cuda()
    ...
    train_set = ot.OT('train',transform=input_transform, target_transform=target_transform)
    train_loader = DataLoader(train_set, batch_size=2, num_workers=4, shuffle=True)
    val_set = ot.OT('val',transform=input_transform, target_transform=target_transform)
    val_loader = DataLoader(val_set, batch_size=1, num_workers=4, shuffle=False)

    criterion = CrossEntropyLoss2d(size_average=False, ignore_index=ot.ignore_label).cuda()
    ...
    open(os.path.join(ckpt_path, exp_name,
                      str(datetime.datetime.now()).replace(' ', '_').replace('.', '_').replace('-', '_').replace(':',
                                                                                                                 '_') + '.txt'),
...
def train(train_loader, net, criterion, optimizer, epoch, train_args):
    ...
    for i, data in enumerate(train_loader):
        ...
        assert outputs.size()[1] == ot.num_classes
...
def validate(val_loader, net, criterion, optimizer, epoch, train_args, restore, visualize):
    ...
    acc, acc_cls, mean_iu, fwavacc = evaluate(predictions_all, gts_all, ot.num_classes)
    ...
    if mean_iu > train_args['best_record']['mean_iu']:
        ...
        for idx, data in enumerate(zip(inputs_all, gts_all, predictions_all)):
            ...
            gt_pil = ot.colorize_mask(data[1])
            predictions_pil = ot.colorize_mask(data[2])
            if train_args['val_save_to_img_file']:
